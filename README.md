...
Run `sudo apt-get install gufw` to install Uncomplicated Firewall (UFW) with the graphical user interface. Type `Y` when prompted.
...
If you are working at the command line, check the firewall status with `sudo ufw status` to check to see if your rules have been enabled. If you are working at the GUI, close the firewall window. 

You can also use `sudo ufw status` to check the firewall status.
...
Use `sudo apt-get install unattended-upgrades` to install the unattended-upgrades software

Type `Y` to continue.

Use `sudo nano /etc/apt/apt.conf.d/10periodic` to open the `10 periodic` file.
...
Scroll down to find the uncommented line `Unattended-Upgrade::Origins- Pattern{`. We want to make a few edits here. 

Uncomment or add the following lines in this section:
```
"o=Raspbian,n=jessie";
...
Install Lynx using `sudo apt-get install lynx`

With Lynx installed, use the command `lynx` followed by a web address to navigate the web. Try navigating to a few of your favorite websites.
...
Install Apache using `sudo apt-get install apache2 -y`
...
Many applications will use Apache’s `Mod_Rewrite` capabilities. This function allows you to create human-readable URLs (i.e. gizmo.com/latest_and_greatest/specific_gadgets/exactly_what_youre_looking_for VS. http://www.gizmo.com/gp/itemB004RYVI0Q/ref=as_li_ss_tl?) More on this later. 

Activate this rewrite function using `command sudo a2enmod rewrite`

Restart Apache using the requested command. Remember to use `sudo`

Take a look at the hosts file using `sudo nano /etc/hosts`
...
`localhost` always refers to your local computer. This is your Pi. `raspberrypi` also refers to your computer. Remember that at the command line you are `pi@raspberrypi`, or the user Pi on the computer `raspberrypi`. 
...
Test the Apache installation using lynx and our local host. At the prompt type, `lynx localhost`. You should see the Apache2 Debian Default Page
...
Now we are going to configure the server using the `.conf` file (aka the configuration file).

At the command line use `sudo nano` to open `/etc/apache2/apache2.conf`
 
This file configures the behavior of our server. The commented lines explain the function of the commands that follow. 

Remember that if we were to set a static IP address (different from 127.0.0.1) and allow web traffic to find our page we would have other computers accessing ours. These settings control how our server reacts when handling requests from multiple machines.

We are going to add a few settings that would ensure that our Pi is not overloaded by requests on the web. Scroll down to the bottom of the file and add a line of blank space, then add the following lines:
```
# User Customization
...
```
Save the file and restart your server using `sudo service apache2 restart`.
...
Let’s add a host `mysite.dev`. (You can use any name that you would like here. I suggest using the .dev extension rather than .com, .org, .net, etc so that your web browser does not attempt to connect to already existing sites out there in the world.)

Reopen the hosts file `sudo nano /etc/hosts`

Now we will append the file a bit. The entire block of 127.0.0.0 addresses (127.0.0.1 through 127.255.255.255) have been reserved for loopback to local hosts, meaning that we can assign different domain names to different IP localhost IP addresses. So, let’s add mysite.dev to our hosts file.

Below the `raspberrypi` line, add `127.0.1.2 mysite.dev`. Exit and save.

Check to make sure that your new domain name works using `lynx mysite.dev`. You should see the default Apache page.

The web browser will try to resolve to the www, so make sure to use the full URL http://mysite.dev/ when testing your domain name in the GUI web browser.

At the moment, the system is defaulting to Apache’s default web page. We will want to add our own content, but we need a place to do this.

At the command prompt, navigate to the `www` folder `cd /var/www/html`.

Use `ls` to see the contents. You should see an `index.html` file.

Use `sudo nano` to open this file.
...
What you see here is the html file containing the Apache Default page. This is the file that opens when you request to see `mysite.dev`. Exit `nano`.

We are going to create a new folder to hold our web files. Navigate back to the `www` folder using `cd ..`

<blockquote>Q10: What does this command do?</blockquote>

We should now be back in the `/var/www folder`. You can confirm this by looking at the command prompt. You should see `pi@raspberrypi:/var/www$`.

Create a new directory with the name of our site using `sudo mkdir mysite.dev`

Navigate to the directory with your new folder `cd mysite.dev`

We are going to make another directory that will hold all of the documents that we would like to share with the public on our site. `sudo mkdir public_html`

Navigate to the directory with your newly-created folder `cd public_html`.

Use the print working directory command (`pwd`) to see the current directory. The command should return `/var/www/mysite.dev/public_html`.

Create a simple HTML page for our new `mysite.dev` site using `sudo nano index.html`

Add some content to the <body> section of this new HTML file.
```HTML
  <html>
    <body>Welcome to mysite.dev</body>
  </html>
```
Save and close the file. We can update this later with your files from the HTML project. Don’t worry about it for now.

Open `mysite.dev` in lynx. You’ll still see the Apache Default page. 

We have to tell new site that we would like to make available to the web. We do this by creating a new configuration file for our website. We’ll do this in the Apache `sites-available` file.

Use `sudo nano /etc/apache2/sites-available/mysite.conf` to open a new file.

In the new blank file, add the following:
...
Enable the site using the `command sudo a2ensite mysite.conf`

Restart Apache as prompted. Remember to use `sudo`.

Open `mysite.dev` in lynx. You should now see the text from the html file that you created a few steps ago.
...
2. use `mkdir` to create a `php` directory
3. navigate to the newly-created `php` directory
4. use `touch` to create an `error.log` file in the newly created `php` directory.
...
Enter the following php code into the blank file:
```php
<?php
...
You have just created a basic php file. If you look at the contents of the `public_html` folder, you should now have an `index.html` and `index.php` file in this folder.

Now open http://mysite.dev/index.php in lynx. You should see `Hello World` on the screen.
...
If you just navigate to `mysite.dev` you will see the html file that we created earlier. This is because Apache is defaulting to html. We have to tell the server to default to php by changing the settings. We don’t need to worry about this now.
...
Start with `sudo apt-get install mariadb-server -y`

When the install is complete, restart apache `sudo service apache2 restart`

We need to take a few steps to secure our MySQL databases.
1. run `sudo mysql_secure_installation`
2. hit <enter> when prompted for a password. We haven't set up a password yet.
3. type <y> when prompted to change the root password
4. enter a password **STORE THIS PASSWORD FOR LATER ACCESS**
...
Next, we are going to create a database and grant other users permission to use the databases.
1. Log in using `mysql –u root –p`. This commands requests to login as the root user with a password.
2. When prompted, enter your mysql password.
3. You should now see the mysql prompt `mariadb>`. This means that we are now working within MySQL and not the Bash Shell.

Now we are going to create a new database and a new user that can have access to that database.
1. First create a new database called csc2020: `create database csc2020;`
...
2. Now you’ll create a new user of the csc2020 called `csc_user` that is identified by a new password. 
3. In this case substitute a password for the word password: `grant all on csc2020.* to ‘csc_user’ identified by ‘[INSERT A PASSWORD]’;`
4. You have now configured MySQL and can pass on these database credentials to other users of your database. 
5. To exit use the command `quit`.
...
2. Open the file in nano `sudo nano /etc/phpmyadmin/apache.conf`
3. Under the directory section, add the line `AllowOverride All` under `Directory Index`
```php
  <Directory /usr/share/phpmyadmin> 
    ...
    AllowOverride All
```
We have now allowed configuration with the `.htaccess` file. Now we can set up a user whose login would be required to access the phpmyadmin login page, adding another layer of security.
1. Open the `.htaccess` file with nano `sudo nano /usr/share/phpmyadmin/.htaccess`
2. Set up the user authorization within the blank `.htaccess` file with the following code:
```
AuthType Basic
...
Now we need to create the `.htpasswd` file. Use the `htpasswd` command to place the `.htpasswd` file in the `/etc` directory. 

Make sure that your `.htaccess` file and the path to the file are the same. 

Be sure to put this somewhere where it is not accessible from the browser (i.e. do not use `/var/www/mysite.dev/public_html`).

Create a new user with the following command. Replace `username` with the user name that you would like to use: `sudo htpasswd –c /etc/.htpasswd username`

Provide and confirm a password. Remember to create a strong password.

Restart apache `sudo service apache2 restart`

You have just created a separate user name and password to access the phpMyAdmin login page.
...
Open the web browser and navigate to http://mysite.dev/phpmyadmin.

If your setup was successful you should be prompted for a user name and password in a pop-up before you are granted access to the phpMyAdmin site. This is the .htpasswd user and password that we just created.

Now you are at the phpMyAdmin login. Here you need to provide the username and password for the `csc2020` database.

If successful, you should now have access to phpMyAdmin. This tool allows you to administer your databases and users from a GUI interface rather than from the command line. We’ll revisit this later this term.

Logout by clicking on the door icon next to the home icon.

Congrats! You have now successfully installed a LAMP stack on your Pi. You can host webpages by placing .html or .php files into the public_html directory for the site. 

In our next project, we will install WordPress and explore how the elements of the L-A-M-P stack work together.
